...
def hand_dist(XA, XB):
    ...
    for i in range(L):
        ...
        distance[i, 1] = (
            (1 - ((dot / ((denom_a ** 0.5) * (denom_b ** 0.5))))) ** 2
        ) ** 0.25
...
@njit
def chessboard_division_njit(
    Uniquesample, MMtypicality, grid_trad, grid_angl, distancetype
):
    """
    ...
    for i in range(W, L):
        ...
        if COUNT >= 1:
            ...
            BOX_miu[SQ[b]] = (
                (BOX_S[SQ[b]] - 1) / BOX_S[SQ[b]] * BOX_miu[SQ[b]]
                + Uniquesample[i] / BOX_S[SQ[b]]
            )  # Eq. 21a
            BOX_X[SQ[b]] = (BOX_S[SQ[b]] - 1) / BOX_S[SQ[b]] * BOX_X[SQ[b]] + np.sum(
                Uniquesample[i] ** 2
            ) / BOX_S[SQ[b]]
...
@njit(fastmath=True)
def ChessBoard_PeakIdentification_njit(
    BOX_miu, BOXMT, NB, grid_trad, grid_angl, distancetype
):
    """
...
@njit(fastmath=True)
def cloud_member_recruitment_njit(
    ModelNumber, Center_samples, Uniquesample, grid_trad, grid_angl, distancetype
):
    """
...
def SelfOrganisedDirectionAwareDataPartitioning(Input):
    ...
    BOX, BOX_miu, BOX_X, BOX_S, BOXMT, NB = chessboard_division_njit(
        Uniquesample, GD, grid_trad, grid_angl, distancetype
    )

    bar.update("  ChessBoard_PeakIdentification_njit")

    Center, ModeNumber = ChessBoard_PeakIdentification_njit(
        BOX_miu, BOXMT, NB, grid_trad, grid_angl, distancetype
    )

    bar.update("  cloud_member_recruitment_njit")

    IDX = cloud_member_recruitment_njit(
        ModeNumber, np.array(Center), data, grid_trad, grid_angl, distancetype
    )
