...
app.layout = html.Div(
    [
        ...
        dcc.Graph(id="graph"),
        html.P("batch size:"),
        dcc.Slider(id="slider", min=100, max=500, value=100, marks={
            100: '100',
            144: '144',
            188: '188',
            233: '233',
...
def update_bar_chart(batch_size, encoding_dim):
    df = results_dict[str(batch_size)][str(encoding_dim)]
    df = df.sort_values(by="eval")

    fig = px.line(results_dict[str(batch_size)][str(encoding_dim)], x="eval", y="RMSE", color='target')
